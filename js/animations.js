...
// Adds pull-to-refresh to a scrollable .screen container. Only
// activates when the screen is scrolled to the very top, so it
// doesn't fight with normal scrolling. onRefresh should return a
// promise (e.g. async load() + re-render).
export function initPullToRefresh(screenEl, onRefresh, options = {}){
    if(!screenEl || screenEl._ptrBound) return;
    ...
    const setPull = (px) => {
        ...
        indicator.style.transform = `translate(-50%, ${-20 + progress * 20}px) scale(${0.6 + progress * 0.4})`;
    };

    screenEl.addEventListener('pointerdown', (e) => {
        if(refreshing || screenEl.scrollTop > 0) return;
        startY = e.clientY;
        pulling = true;
    });
    ...
    screenEl.addEventListener('pointermove', (e) => {
        ...
        if(screenEl.scrollTop > 0){ pulling = false; setPull(0); return; }

        const delta = e.clientY - startY;
        if(delta <= 0){ currentPull = 0; setPull(0); return; }

        // Rubber-band resistance past maxPull.
        currentPull = delta < maxPull ? delta : maxPull + (delta - maxPull) * 0.2;
        e.preventDefault();
        setPull(currentPull);
    }, { passive: false });
    ...
    screenEl.addEventListener('pointerup', endPull);
    screenEl.addEventListener('pointercancel', endPull);
}